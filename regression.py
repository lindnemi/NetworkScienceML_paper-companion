...
def run_regression(model, scaler, model_name, with_interactions, random_state):
    ...
    if with_interactions:
        X20 = X20.drop(columns=['maximal_line_load_post_dc', 'backup_capacity'])
        X100 = X100.drop(columns=['maximal_line_load_post_dc', 'backup_capacity'])
        Xtex = Xtex.drop(columns=['maximal_line_load_post_dc', 'backup_capacity'])
    ...
    for X, y, nodes_per_grid, X_path, y_path in [
        ...
        for train_fraction in [0.0007, 0.007, 0.07, 0.7]:
            ...
            save_model(model_name, model, scaler,
                task_name, random_state, nodes_per_grid, work_dir, X_path, y_path, X_train.index, None, X20_test.index,
                X100_test.index, ttrain, with_interactions, train_r2=train_r2,
                ev20_r2=ev20_r2, ev100_r2=ev100_r2, evTex_r2=evTex_r2, sparse_predictors=sparse_predictors)
